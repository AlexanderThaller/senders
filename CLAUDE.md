...
`deploy/k8s` is kustomize: `base` plus `overlays/production` and
`overlays/single-node`. Validate a change with
`kubectl apply --dry-run=client -k deploy/k8s/overlays/<name>` — client-side, so
it needs no cluster. The Secret is deliberately not generated by kustomize;
`deploy/k8s/README.md` says why, and records the two ingress settings that
otherwise break uploads.
